...
def _convert_height(element: dict) -> float:
    ...
    if unit in ("cm", "centimetres", "centimeters"):
        ...
        logging.warning(
            f"Unknown unit of measurement for the hero's height: {element['name']}"
        )
        return -1
...
def get_highest_hero(gender: str, is_working: bool) -> str:
    ...
    for element in all_heroes_data:
        ...
        if (
            element["appearance"]["gender"].lower() == gender.lower()
            and is_hero_working == is_working
        ):
    ...
    if sorted_data:
        hero_name = (
            sorted_data[0]["name"] if sorted_data[0]["name"] else "Nameless hero"
        )
